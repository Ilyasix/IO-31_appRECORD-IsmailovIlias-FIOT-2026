...
### 2.3 Причини вибору MySQL та Sequelize
Для реалізації серверної частини було обрано `MySQL`, оскільки для системи задач природно підходить реляційна модель даних зі зв'язками між користувачами, групами задач і самими задачами. Такий підхід забезпечує:
- цілісність даних;
- підтримку зовнішніх ключів;
- зручне масштабування структури таблиць;
- просте формування вибірок із пов'язаними сутностями.

Використання `Sequelize` додатково спрощує:
- опис моделей у коді;
- роботу зі зв'язками `one-to-many`;
- повторне використання запитів;
- перехід від ручного SQL до більш структурованого прикладного рівня.

### 2.4 Моделі та зв'язки між сутностями
...
### 2.5 Забезпечення цілісності даних
Під час проєктування таблиць і моделей окрему увагу було приділено тому, щоб у системі не виникали записи без змістовного контексту. Саме тому задача зберігається не ізольовано, а разом із прив'язкою:
- до конкретного користувача;
- до конкретної групи задач;
- до статусу;
- до пріоритету.

Такий підхід важливий для робочого середовища, тому що дозволяє:
- уникати "нічийних" задач;
- забезпечити класифікацію записів;
- спростити фільтрацію та подальший аналіз.

### 2.6 Реалізація операцій над задачами
...
### 2.7 Отримання та оновлення даних
...
### 2.9 Практичне значення отриманої структури
Сформована база даних уже на цьому етапі відображає прикладну природу сервісу `TaskFlow`. Вона придатна не лише для простого зберігання списку задач, а й для:
- побудови багатокористувацької логіки;
- подальшого введення ролей і авторизації;
- відображення задач у вигляді дошки або фільтрованих списків;
- додавання вкладень і журналів активності.

Отже, друга лабораторна робота заклала не просто технічну БД, а структуровану основу всього подальшого застосунку.

### 2.10 Підсумок виконання
