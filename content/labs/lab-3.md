...
### 2.3 Ролі користувачів і модель доступу
Для `TaskFlow` було важливо не просто ідентифікувати користувача, а й розрізняти його рівень доступу. Тому в системі використовуються ролі:
- `user`;
- `admin`.

Звичайний користувач орієнтований на роботу зі своїми задачами та робочими маршрутами, а адміністратор має змогу:
- переглядати список користувачів;
- контролювати загальну структуру системи;
- працювати з ширшим набором службових маршрутів.

Такий підхід дозволяє одразу будувати API з урахуванням багатокористувацького сценарію.

### 2.4 Реєстрація та перевірка коректності даних
...
### 2.5 Авторизація та токени доступу
...
### 2.6 Логіка access token і refresh token
Після входу користувач отримує два токени з різним призначенням:
- `access token` використовується для доступу до захищених маршрутів;
- `refresh token` застосовується для оновлення сесії без повторного введення пароля.

У практичному сенсі це підвищує зручність роботи із системою та робить механізм доступу ближчим до реальних серверних рішень, де короткоживучий токен використовується для API, а довший токен зберігається для оновлення авторизації.

### 2.7 Захист маршрутів
...
### 2.8 Робота з профілем та сесією
...
### 2.9 Валідація та обробка помилкових сценаріїв
Окрім позитивних сценаріїв входу та реєстрації, було враховано й типові помилки:
- порожні поля;
- невалідний email;
- надто короткий пароль;
- невірні облікові дані;
- відсутній або прострочений токен.

Завдяки цьому API повертає не випадкові помилки, а зрозумілі структуровані відповіді, що є важливим як для тестування, так і для подальшого підключення клієнтської частини.

### 2.10 Поєднання авторизації з робочими маршрутами
...
### 2.11 Практичне значення реалізованого підходу
Після впровадження авторизації `TaskFlow` переходить від простого сервісу з таблицями до керованого багатокористувацького API. Це означає, що система вже:
- розпізнає конкретного користувача;
- контролює доступ до критичних дій;
- відокремлює звичайні маршрути від адміністративних;
- підтримує життєвий цикл сесії.

Саме це створює основу для безпечної роботи з задачами в реальному сценарії.

### 2.12 Підсумок реалізації
